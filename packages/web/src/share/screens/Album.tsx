...
const R = createResource((q: string) => agent.album(q));
function Album({ match }: any) {
  const albumId = match.params.id;
  const album = R.read(albumId);
    ...
    <ScrollView style={styles.container}>
      <View style={styles.back}>
        <Image
          style={styles.bg}
          source={{
            uri: `//y.gtimg.cn/music/photo_new/T002R800x800M000${albumId}.jpg`,
          }}
        />
      </View>
      <View style={styles.play}>
        <PlayButton onPress={() => play(album.songs[0])} />
      </View>
      {album.songs.map((song: any) => (
